...
- config.yaml：非机密配置主文件（模型、显示、终端、压缩、委托、辅助任务等）
...
## 核心组件
- 配置文件与来源
  - config.yaml：集中存放非机密配置项，如 model、display、terminal、compression、delegation、auxiliary 等
  - .env：存放机密信息，作为回退来源
...
- 经验法则
  ...
  - 其他（模型、终端后端、压缩、内存限制、工具集）放入 config.yaml
...
## 结论
Hermes Agent 的配置管理以 config.yaml 为核心，配合 .env 与环境变量实现灵活覆盖与安全隔离。CLI、网关与前端通过统一的 API 与 schema 实现配置的可视化、校验与热重载。遵循“机密进 .env、其他进 config.yaml”的经验法则，结合迁移、备份与恢复策略，可在多环境中稳定地管理配置。
